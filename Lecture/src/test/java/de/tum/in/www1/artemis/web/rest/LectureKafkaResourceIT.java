...
class LectureKafkaResourceIT {
    ...
    void setup() {
        ...
        LectureKafkaResource kafkaResource = new LectureKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/lecture-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/lecture-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
